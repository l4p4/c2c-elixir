defmodule C2cWeb.ConnCase do
  ...
  by setting `use C2cWeb.ConnCase, async: true`, although
  ...
  using do
    quote do
      ...
      import C2cWeb.ConnCase

      alias C2cWeb.Router.Helpers, as: Routes
  ...
  setup tags do
    C2c.DataCase.setup_sandbox(tags)
  ...
  def log_in_user(conn, user) do
    token = C2c.Accounts.generate_user_session_token(user)
  ...
  def create_user_and_assign_valid_jwt(%{conn: conn}) do
    user = C2c.AccountsFixtures.user_fixture()
    {:ok, jwt, _full_claims} = C2c.Guardian.encode_and_sign(user, %{})
